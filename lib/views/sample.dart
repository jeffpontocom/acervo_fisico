...
class _RegistrationPageState extends State<RegistrationPage> {
  ...
  Future<List<ParseObject>> doListRegistration() async {
    QueryBuilder<ParseObject> queryRegistration = QueryBuilder<ParseObject>(
        ParseObject(registrationType.className ?? 'Genre'));
    final ParseResponse apiResponse = await queryRegistration.query();

    if (apiResponse.success && apiResponse.results != null) {
      //return apiResponse.results;
      List<ParseObject>? res = apiResponse.results!.cast<ParseObject>();
      return res;
    } else {
      return [];
    }
  ...
  Future<void> doSaveRegistration(String name) async {
    final registration = ParseObject(registrationType.className ?? 'Genre')
      ..set('name', name);
...
class _CheckBoxGroupWidgetState extends State<CheckBoxGroupWidget> {
  ...
  Widget checkBoxTile(ParseObject data) {
    return CheckboxListTile(
      title: Text(data.get<String>('name') ?? 'Erro'),
                  ...
                  return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ExpansionTile(
                          title: Text(varName ?? 'Erro'),
                          //tilePadding: Icon,
                          children: [BookTile(varPublisher.objectId ?? 'Erro')],
                        );
                      });
                }
            }
                  ...
                  return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          title: Text(book.get<String>('title') ?? 'Erro'),
                          onTap: () {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
